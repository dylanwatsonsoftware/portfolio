...
### Integration
Integration tests are larger in scope than unit tests. They often spin-up a real server but usually only connect to an in-memory database. Any calls to other APIs/services are mocked (If the external services are not mocked, this would be classed as an "end to end" test - not in scope for this article but probably flakey). Integration tests test more of the system than unit tests and are closer (but not exactly) to "production likeness". However, they come at a cost - they are often slower and harder to debug than unit tests. Despite this, they may make it easier to reproduce bugs and prove functionality of a system.

### Sociable Unit Tests
Sociable Unit Tests are an attempt at a happy medium between unit tests and integration tests. They attempt to test a larger portion of system than unit tests. Like integration tests, the only mocked dependencies should be things outside of the APIs "sphere of influence" - things like network calls or databases I.e API clients or Repository services. Server/Framework/Spring specific code should also be avoided - we really only want to test OUR code.

![Sociable vs Solitary Unit Tests](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/56loadbf1f4vscy7iafm.png)
_A comparison of 2 common styles of unit tests_

## What are we after?
Basically, we just want our tests to be:
- **Fast**
...
![Test Landscape](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/ih6illj0zv8afz6qiu2n.png)
...
![Nelson laughing at dependencies](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/y0zbgblhcdwu1god8kfa.jpg)
_Don't do this!_
...
```
Basically, the ProductService has a PricingEngine which it delegates to, to calculate the price.

Let's take a look at how we'd test it in both approaches (solitary and sociable).

# Solitary Approach
You will probably recognise the following approach to testing as it has become one of the most popular in recent years. To isolate our system-under-test, we will mock it's dependencies, like so:
```java
// Solitary Unit Test
...
```
But if we actually think about it, the only thing this is testing is that we call a specific method on the engine.

In fact, if we refactor ProductService to call a different (but equivalent) method, our test would fail. Now not all mock-heavy tests are this terrible but it happens far to often.
...
The core thing we really want to test is that we can get a price for a given product.. so let's give it another go with that in mind!
...
# Make a change
Now what if we now add the requirement for our price to include a discount based on the customer?
...
On a suite of hundreds or thousands of tests, the affect sociable-style tests can have on refactoring is huge!

Obviously, we'd want to add other tests or assertions for this new functionality but the simple act of using real dependencies in our tests puts us in a much better place from the start.

# Conclusion
Sociable unit tests are an approach that it works just as well for small scoped unit tests as it does for integration testing whole pieces of a microservice. Whilst you may still find the need to mock external dependencies such as APIs or slow, non-deterministic ones like a database or filesystem, attempting to use more real dependencies in your tests can have a huge impact on both the speed and usefulness of your tests.
